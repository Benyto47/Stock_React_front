...
function Articles() {
  ...
  const initialPages = () => {
    axios
      .get("http://localhost:8181/stock/articles/admin", {
        params: {
          page: 0,
          size: 10,
        },
  ...
  const handleDelete = (e) => {
    const confirmation = window.confirm(
      "êtes vous sûr de supprimé l'article ? "
    );

    if (confirmation) {
      console.log("delete : ", e);
      axios
        .delete(`http://localhost:8181/stock/articles/${e}`)
        .then((response) => {
          console.log(
            " article supprimé  :",
            e,
  ...
  return (
    <div className="min-w-6xl mx-auto">
      <div className="mb-5 mt-7">
        <h1 className="mb-5 text-blue-700"> Admin {">"} Articles en Stock </h1>
      ...
      <table className="mb-5 table-auto border-spacing-4 border-collapse bg-white border-purple-700  sm:block hidden">
        <thead>
          <tr className="bg-blue-400 text-white italic">
            ...
            <th className="px-3 border border-slate-200">Quantités stock</th>
            <th className="px-14 border border-slate-200">Date création</th>
                ...
                <tr>
                  ...
                  <td className="">
                    <div className="px-5 hover:text-white duratio-300 hover:bg-pink-300 bg-white flex space-x-3 p-3 rounded-xl">
                      ...
                      <button className="px-3 hover:text-blue-700">
                        <Link
                          to={`/admin/articles/stock/update/${item.id}`}
